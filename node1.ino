...
void loop() {
  ...
  if(client.publish("kelembapan-node1", String(DHT.humidity).c_str())){
    ...
    delay(10);
    client.publish("kelembapan-node1", String(DHT.humidity).c_str());
  }
 
  if(client.publish("suhu-node1", String(DHT.temperature).c_str())){
    ...
    client.connect("ESP8266Client1", mqtt_user, mqtt_password);
    delay(10);
    client.publish("suhu-node1", String(DHT.temperature).c_str());
  }
  
  if(client.publish("moisture-node1", String(moisture_percentage).c_str())){
    ...
    client.connect("ESP8266Client1", mqtt_user, mqtt_password);
    delay(10);
    client.publish("moisture-node1", String(moisture_percentage).c_str());
  }
  
  if(client.publish("hujan-node1", String(rainDigitalVal).c_str())){
    ...
    client.connect("ESP8266Client1", mqtt_user, mqtt_password);
    delay(10);
    client.publish("moisture-node1", String(rainDigitalVal).c_str());
  }
